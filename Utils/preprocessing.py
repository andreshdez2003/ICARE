...
def process_dataframe(df: pd.DataFrame, window_length: int = 15, polyorder: int = 2) -> pd.DataFrame:
    ...
    # Paso 4: Eliminación de NaNs
    nan_rows = processed_df[resistance_columns].isnull().any(axis=1)
    if nan_rows.any():
        indices_eliminados = processed_df[nan_rows].index.tolist()
        print(f"⚠️ Filas eliminadas por contener NaNs: {indices_eliminados}")
        processed_df = processed_df.drop(index=indices_eliminados)
